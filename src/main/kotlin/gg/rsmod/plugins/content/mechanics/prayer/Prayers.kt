...
object Prayers {
    ...
    val DISABLE_PROTECTION = TimerKey()
    ...
    /**
     * Toggles protection prayers immediately and disables them for a specified amount of cycles.
     */
    fun disableProtection(p: Player, cycles: Int) {
        p.timers[DISABLE_PROTECTION] = cycles
        Prayer.values.forEach { deactivate(p, it) }
    }
    ...
    suspend fun toggle(it: QueueTask, prayer: Prayer) {
        ...
        if (p.isDead() || !p.lock.canUsePrayer()) {
            ...
            return
        } else if (prayer.group == PrayerGroup.PROTECTION && p.timers.has(DISABLE_PROTECTION)) {
            p.syncVarp(ACTIVE_PRAYERS_VARP)
